...
def show_highest_rated_art():
    best = ArtworkGallery.objects.all().order_by('rating').last()
    return f"{best.art_name} is the highest-rated art with a {best.rating} rating!"
...
def update_to_512_GB_storage():
    laptop = Laptop.objects.all().filter(brand__in=('Asus', 'Lenovo'))
...
def update_to_16_GB_memory():
    laptop = Laptop.objects.all().filter(brand__in=('Apple', 'Dell', 'Acer'))
    for lap in laptop:
        lap.memory = 16
        lap.save()
...
def update_operation_systems():
    Laptop.objects.all().filter(brand='Asus').update(operation_system='Windows')
    Laptop.objects.all().filter(brand='Apple').update(operation_system='MacOS')
    Laptop.objects.all().filter(brand__in=['Dell', 'Acer']).update(operation_system='Linux')
    Laptop.objects.all().filter(brand='Lenovo').update(operation_system='Chrome OS')
...
def delete_chess_players():
    players = ChessPlayer.objects.all().filter(title="no title")
    for player in players:
        player.delete()


def change_chess_games_won():
    players = ChessPlayer.objects.all().filter(title='GM')
    for player in players:
        player.games_won = 30
        player.save()
...
def update_low_calorie_meals():
    Meal.objects.all().filter(meal_type__in=('Breakfast', 'Dinner')).update(calories=400)


def update_high_calorie_meals():
    Meal.objects.all().filter(meal_type__in=('Lunch', 'Snack')).update(calories=700)


def delete_lunch_and_snack_meals():
    Meal.objects.all().filter(meal_type__in=('Lunch', 'Snack')).delete()
