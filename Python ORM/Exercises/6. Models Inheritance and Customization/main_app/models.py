...
class Message(models.Model):
    ...
    def reply_to_message(self, reply_content: str):
        new_message = Message.objects.create(sender=self.receiver,
...
class StudentIDField(models.PositiveIntegerField):
    ...
    def get_prep_value(self, value):
        to_validate = self.to_python(value)
        if to_validate <= 0:
...
class BaseReservation(models.Model):
    ...
    def save(self, *args, **kwargs) -> None:
        ...
        for reservation in self.__class__.objects.all():
            if reservation.end_date >= self.start_date and reservation.start_date <= reservation.end_date:
                raise ValidationError(f'Room {self.room.number} cannot be reserved 1')
