...
from main_app.validators import MovieRatingValidator, ReleaseYearValidator
...
class VideoGame(models.Model):
    ...
    release_year = models.PositiveIntegerField(validators=[ReleaseYearValidator])
    rating = models.DecimalField(max_digits=2, decimal_places=1, validators=[MovieRatingValidator])
...
class Invoice(models.Model):
    ...
    def get_invoice_with_billing_info(cls, invoice_number: str):
        result = cls.objects.get(invoice_number=invoice_number)
        return result
...
class Task(models.Model):
    ...
    def completed_mid_priority_tasks(cls):
        return cls.objects.filter(priority='Medium',
                                  is_completed=True)
    @classmethod
    def search_tasks(cls, query: str):
        return cls.objects.filter(title__contains=query)

    @classmethod
    def recent_completed_tasks(cls, days: int):
        return cls.objects.filter(is_completed=True,
