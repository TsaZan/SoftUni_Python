...
def find_authors_nationalities() -> str:
    ...
    result = [f'{a.first_name} {a.last_name} is {a.nationality}' for a in authors_with_nationality]
    return "\n".join(result)
...
def order_books_by_year() -> str:
    books_ordered_by_year = Book.objects.order_by('publication_year', 'title')
    result = [f"{b.publication_year} year: {b.title} by {b.author}" for b in books_ordered_by_year]
    return "\n".join(result)


# print(order_books_by_year())

def delete_review_by_id(id) -> str:
    review = Review.objects.filter(id=id)
    reviewer_name = review[0].reviewer_name
    review.delete()
...
def change_reviewer_name(reviewer_name, new_reviewer_name) -> QuerySet:
    reviews = Review.objects.filter(reviewer_name=reviewer_name)
    for r in reviews:
        r.reviewer_name = new_reviewer_name
        r.save()
    # return Review.objects.filter(reviewer_name=new_reviewer_name)
